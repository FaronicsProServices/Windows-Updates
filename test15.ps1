...
function Get-ProfileFolders {
    param($root)
    # Include all profiles including Default & Public
...
function Resize-CPartitionAndCreateNew {
    ...
    if ($NewPartitionSizeBytes -gt $supported.SizeMax) {
        Write-Warning "Requested partition size too large. Shrinking max available space instead."
        $NewPartitionSizeBytes = $supported.SizeMax
    }

    $currentSize = $cPart.Size
    $targetCSize = $currentSize - $NewPartitionSizeBytes
    ...
    # Fully silent format (no prompts)
...
function Copy-Profiles {
    ...
    foreach ($folder in Get-ChildItem $sourceRoot -Directory -Force) {
        if ($folder.Name -eq 'All Users') { continue }  # skip special link

        $src = $folder.FullName
        $dst = Join-Path $destRoot $folder.Name

        # Check if profile is loaded (only applies to normal users)
...
function Update-RegistryProfilePaths {
    ...
    # Update Default profile path in registry
    ...
    # Update Public profile path in registry
...
Write-Output "Profiles size: $(BytesToGB $totalBytes) GB, required partition: $(BytesToGB $required) GB"
