...
param(
    [string]$SourcePath = "\\pdc\Deploy2",   # Folder containing setup.exe
...
# --- FIX 1: Create local directory before logging ---
...
try {
    # --- FIX 2: Copy from folder, not exe ---
    Copy-Item -Path "$SourcePath\*" -Destination $LocalPath -Recurse -Force -ErrorAction Stop
