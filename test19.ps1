...
# Calculate 20% buffer
$buffer = [int64]($totalBytes * 0.2)
$required = $totalBytes + $buffer

# Ensure we do not exceed max shrinkable space
$cShrinkInfo = Get-PartitionSupportedSize -DriveLetter C
$maxShrinkable = $cShrinkInfo.SizeMax
if ($required -gt $maxShrinkable) {
    Write-Warning "Required partition size exceeds shrinkable space. Reducing to maximum available: $(BytesToGB $maxShrinkable) GB"
    $required = $maxShrinkable
}

Write-Output "Profiles size: $(BytesToGB $totalBytes) GB"
Write-Output "Buffer added (20%): $(BytesToGB $buffer) GB"
