...
function Resize-CPartitionAndCreateNew {
    ...
    $maxShrinkable = $supported.SizeMax
    if ($NewPartitionSizeBytes -gt $maxShrinkable) {
        Write-Warning "Requested partition size $(BytesToGB $NewPartitionSizeBytes) GB exceeds max shrinkable space $(BytesToGB $maxShrinkable) GB. Adjusting to max possible."
        $NewPartitionSizeBytes = $maxShrinkable
    }

    $targetCSize = $cPart.Size - $NewPartitionSizeBytes
    if ($targetCSize -lt 20GB) {
        throw "Not enough free space to shrink C:. Aborting."
    }

    Write-Output "Shrinking C: to create new partition of size $(BytesToGB $NewPartitionSizeBytes) GB..."
...
function Copy-Profiles {
    ...
    foreach ($folder in Get-ChildItem $sourceRoot -Directory -Force) {
        ...
        # Check if profile is loaded (locked NTUSER.DAT)
...
Write-Output "Profiles size: $(BytesToGB $totalBytes) GB, requested partition: $(BytesToGB $required) GB"
